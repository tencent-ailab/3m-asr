...
class ConformerEncoderLayer(nn.Module):
    ...
    def __init__(
        ...
        size: int,
        self_attn: torch.nn.Module,
        feed_forward: Optional[nn.Module],
    ...
    def forward(
        ...
        if self.feed_forward_macaron is not None:
            ...
            if not self.normalize_before:
                x = self.norm_ff_macaron(x)
        # attention module and conv module
        x, mask, new_cnn_cache = self.attention_conv(
                x, mask, pos_emb, mask_pad, output_cache, cnn_cache)
        # feed forward module
        residual = x
        if self.normalize_before:
            x = self.norm_ff(x)
...
class FmoeConformerLayer(ConformerEncoderLayer):
    """Conformer encoder layer with MoE(mixture-of-experts),
    where the feed-forward module is `FmoeCatEmbedFeedForward` instance
    """
    def forward(
        self,
        x: torch.Tensor,
        ...
        aux_loss_res = []
        # whether to use macaron style, positionwise-feed-forward
        if self.feed_forward_macaron is not None:
            residual = x
            if self.normalize_before:
                x = self.norm_ff_macaron(x)
        ...
        x, mask, new_cnn_cache = self.attention_conv(
                x, mask, pos_emb, mask_pad, output_cache, cnn_cache)
        # moe feed forward module
        residual = x
        if self.normalize_before:
            x = self.norm_ff(x)
        h2, aux_loss = self.feed_forward(x, embed)
        aux_loss_res.append(aux_loss)
        ...
        if output_cache is not None:
            x = torch.cat([output_cache, x], dim=1)
        return x, aux_loss_res, mask, new_cnn_cache
